...
for i in range(len(files)):
    ...
    if os.path.isfile(filepath):
        ...
        cv2.imwrite('ULAP_TM.jpg', np.uint8(transmission[:, :, 2] * 255))
        plt.subplot(312),plt.imshow(np.uint8(transmission[:, :, 2] * 255))
        plt.show()

        # print('AtomsphericLight',AtomsphericLight)

        cv2.imwrite('ULAP.jpg', sceneRadiance)
        plt.subplot(313),plt.imshow(sceneRadiance)
        plt.show()
